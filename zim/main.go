...
func init() {
	...
	service.ReceiveSvr = service.NewReceiveSvr() //接受handle
	service.UserSvr = service.NewUserSvr()       //用户管理handle
	service.ConnectHub = service.NewConnectHub() //ws连接池

	sys.DbConn = sys.NewDbConn()
	sys.RedisPool = sys.NewRedisPool() //redis连接池
	sys.RpcPool = sys.NewRpcPool()     //rpc连接池
}
...
func serverUserSvr(w http.ResponseWriter, r *http.Request) {
	if !common.ApiKeyCheck(r.FormValue("key"), sys.BaseConf.Get("api").Get("key").MustString(), sys.BaseConf.Get("api").Get("timeSlice").MustInt()) {
		http.Error(w, sys.LangConf.Get("4009").MustString(), 4009)
		return
	}
